...
export const _userList = [...Array(24)].map((_, index) => ({
  _id: _mock.id(index),
  status: randomInArray(['active', 'suspended']),
  firstName: _mock.name.firstName(index),
  middleName: _mock.name.firstName(index),
  lastName: _mock.name.lastName(index),
  email: _mock.email(index),
  phone: _mock.phoneNumber(index),
  phoneSecond: _mock.phoneNumber(index),
  address: _mock.address.fullAddress(index),
  userType: randomInArray(['User', 'Carer']),
  gender: randomInArray(['Male', 'Female']),
  dob: new Date().toDateString(),
  occupation: _mock.address.fullAddress(index),
  height: _mock.number.price(index),
  weight: _mock.number.price(index),
  medicationOption: randomInArray(['Diabetes', 'Arthritis', 'Cancer']),
  culturalBackground: _mock.address.country(index),
  socialEngagement: randomInArray(['Very Active', 'Active', 'Inactive']),
  exercise: randomInArray(['Full Time (Office/Desk Based)', 'Part Time or Casual (Office / Desk Based)']),
}));

export const _userList1 = [...Array(24)].map((_, index) => ({
  id: _mock.id(index),
  avatarUrl: _mock.image.avatar(index),
  name: _mock.name.fullName(index),
  email: _mock.email(index),
