...
import OpenOrderTable from './open_orders/OpenOrderTable';
...
function OpenOrders() {
  ...
  useLegacyEffect(() => {
    const fetch = async () => {
      try {
        const response = await axiosInstance.post('api/v1/users/open-orders');
  ...
  return (
    ...
    >
      <Box sx={{ color: 'grey.200', backgroundColor: '#141A36', borderRadius: 1, p: 2, mb: 2 }}>
        ...
        <OpenOrderTable tableData={tableData} />
      </Box>
    </Box>
  );
}
